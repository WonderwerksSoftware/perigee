...
bool Session::initialize(QQuickWindow* qtWindow)
{
    ...
    if (engine == nullptr) {
        ...
        QString deckError;
        const bool deckAvailable = deckRenderer->initialize(
                engine,
                QUrl(QStringLiteral(
...
bool Session::validateLaunch(SDL_Window* testWindow)
{
    if (!m_Computer->isSupportedServerVersion) {
        emit displayLaunchError(tr("The version of GeForce Experience on %1 is not supported by this build of Perigee. You must update Perigee to stream from %1.").arg(m_Computer->name));
        return false;
    }
    ...
    if (!SdlInputHandler::getUnmappedGamepads().isEmpty()) {
        emitLaunchWarning(tr("An attached gamepad has no mapping and will not be usable. Use the upstream Moonlight help to resolve this."));
    }
...
void Session::exec()
{
...
#else
    std::string windowName = QString(m_Computer->name + " - Perigee").toStdString();
#endif
    ...
    QSvgRenderer svgIconRenderer(QString(":/res/perigee.svg"));
