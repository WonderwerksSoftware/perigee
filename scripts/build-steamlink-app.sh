...
mkdir -p $DEPLOY_FOLDER/steamlink/apps/perigee/bin
cp $BUILD_FOLDER/app/perigee $DEPLOY_FOLDER/steamlink/apps/perigee/bin/ || fail "Binary copy failed!"
cp $SOURCE_ROOT/app/deploy/steamlink/* $DEPLOY_FOLDER/steamlink/apps/perigee/ || fail "Metadata copy failed!"
pushd $DEPLOY_FOLDER
zip -r $INSTALLER_FOLDER/Perigee-SteamLink-$VERSION.zip . || fail "Zip failed!"
popd
