...
pushd $BUILD_FOLDER
dsymutil app/Perigee.app/Contents/MacOS/Perigee -o Perigee-$VERSION.dsym || fail "dSYM creation failed!"
cp -R Perigee-$VERSION.dsym $INSTALLER_FOLDER || fail "dSYM copy failed!"
popd
...
macdeployqt $BUILD_FOLDER/app/Perigee.app $EXTRA_ARGS -qmldir=$SOURCE_ROOT/app/gui -appstore-compliant || fail "macdeployqt failed!"

echo Removing dSYM files from app bundle
find $BUILD_FOLDER/app/Perigee.app/ -name '*.dSYM' | xargs rm -rf

if [ "$SIGNING_IDENTITY" != "" ]; then
  echo Signing app bundle
  codesign --force --deep --options runtime --timestamp --sign "$SIGNING_IDENTITY" $BUILD_FOLDER/app/Perigee.app || fail "Signing failed!"
fi

echo Creating DMG
if [ "$SIGNING_IDENTITY" != "" ]; then
  create-dmg $BUILD_FOLDER/app/Perigee.app $INSTALLER_FOLDER --identity="$SIGNING_IDENTITY" --no-version-in-filename || fail "create-dmg failed!"
else
  create-dmg $BUILD_FOLDER/app/Perigee.app $INSTALLER_FOLDER --no-version-in-filename
  case $? in
    0) ;;
    2) ;;
...
if [ "$NOTARY_KEYCHAIN_PROFILE" != "" ]; then
  ...
  xcrun notarytool submit --keychain-profile "$NOTARY_KEYCHAIN_PROFILE" --wait $INSTALLER_FOLDER/Perigee.dmg || fail "Notary submission failed"

  echo Stapling notary ticket to DMG
  xcrun stapler staple -v $INSTALLER_FOLDER/Perigee.dmg || fail "Notary ticket stapling failed!"
fi

mv $INSTALLER_FOLDER/Perigee.dmg $INSTALLER_FOLDER/Perigee-$VERSION.dmg
